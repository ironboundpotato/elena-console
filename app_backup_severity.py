...
def evaluate_request(user_input: str) -> dict:
    ...
    return make_result(
        ...
        constraints=[],
        drift_flags=[],
        reason="No blocking risk detected by the Phase 4 demo ruleset.",
    )
        ...
        rows.append(
            {
                ...
                "decision": entry.get("decision", entry.get("governance", {}).get("decision", "")),
                "failure_mode": entry.get("failure_mode", entry.get("governance", {}).get("failure_mode", "")),
                "gate": entry.get("execution_gate", entry.get("governance", {}).get("execution_gate", "")),
                "auth": entry.get("authorization_status", entry.get("governance", {}).get("authorization_status", "")),
