            ...
            matches.append(
                {
                    ...
                    "priority": FAILURE_PRIORITY.get(failure_mode, 0),
                    "intent": rule["intent"],
                    "matched_terms": matched_terms,
                    "constraints": rule["constraints"],
                    "drift_flags": rule["drift_flags"],
                    "reason": rule["reason"],
                    "description": FAILURE_MODES[failure_mode],
                }
            )
...
def build_explainability_tree(result: dict) -> dict:
    return {
        ...
        "3_priority_resolution": {
            ...
            "primary_priority_score": result["priority_score"],
...
def build_human_explanation(result: dict) -> list[str]:
    ...
    secondary_line = (
        f"Secondary signals also detected: {', '.join(secondary_signals)}."
        if secondary_signals
        else "No secondary failure signals were detected."
    )
...
def evaluate_request(user_input: str) -> dict:
    ...
    if not text:
        return make_result(
            ...
            matched_terms=[],
            all_detected_modes=[],
            priority_score=FAILURE_PRIORITY["AMBIGUOUS_INTENT"],
        )

    matches = collect_matches(text)

    if matches:
        sorted_matches = sorted(matches, key=lambda item: item["priority"], reverse=True)
        ...
        secondary_signals = [
            match["failure_mode"]
            for match in sorted_matches
            if match["failure_mode"] != failure_mode
        ]
    ...
    return make_result(
        ...
        constraints=[],
        drift_flags=[],
        reason="No blocking risk detected by the Phase 7 demo ruleset.",
        matched_terms=[],
        all_detected_modes=[],
        priority_score=FAILURE_PRIORITY["NONE"],
        secondary_signals=[],
    )
...
def make_result(
    ...
    result = {
        ...
        "secondary_signals": list(dict.fromkeys(secondary_signals)),
        "matched_terms": matched_terms,
        "all_detected_modes": all_detected_modes,
        "authorization_status": authorization_status,
        "actor_trust_level": actor_trust_level,
    ...
    result["explainability_tree"] = build_explainability_tree(result)
    return result
...
def log_run(user_input: str, result: dict) -> None:
    entry = {
        ...
        "explainability_tree": result.get("explainability_tree", {}),
        "governance": result,
    }
        ...
        rows.append(
            {
                ...
                "primary_failure_mode": entry.get("primary_failure_mode", governance.get("primary_failure_mode", "")),
                "priority_score": entry.get("priority_score", governance.get("priority_score", "")),
                "secondary_signals": ", ".join(entry.get("secondary_signals", governance.get("secondary_signals", []))),
...
if rows:
    ...
    with st.expander("Recent run details"):
        for entry in reversed(history[-10:]):
            ...
            st.markdown(f"**Primary Failure Mode:** {governance.get('primary_failure_mode', '')}")
            st.markdown(f"**Priority Score:** {governance.get('priority_score', '')}")
            st.markdown(f"**Secondary Signals:** {governance.get('secondary_signals', '')}")
            ...
            if governance.get("explainability_tree"):
                with st.expander("Explainability tree"):
                    st.json(governance.get("explainability_tree"))
